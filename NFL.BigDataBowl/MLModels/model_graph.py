...
l1 = 0
l2 = 0
ExpectedYards = tf.add(tf.matmul(X, W), b, name='ExpectedYards')
loss = tf.reduce_mean(tf.square(Y - tf.squeeze(ExpectedYards))) / 2 + l2 * tf.nn.l2_loss(W) + l1 * tf.reduce_sum(tf.abs(W))
